...
def average(numlis):
    ...
    count = len(numlis)
    av = total / count
    print("The list average is",av,"and the list count is",count,".")
#%%
"""
End solution
"""
...
def average(nlis):
    listcount = len(nlis)
    total = 0
    for num in range(0,listcount):
        total = total + nlis[num]
        print(nlis[num], end=" ")
    print()
    av = total / listcount
...
def orderFood():
    order = []
    print("Hello, what can I get you?")
    while True:
        nextstr = str(input("Menu item: "))
        if nextstr == "end":
            break
        order.append(nextstr)
    print("Ok, your order is:",order)
#%%